...
bool DataFlowGraphModel::connectionPossible(ConnectionId const connectionId) const
{
    ...
    //model name == "Task Starting Point" regardless of in out port
    if(getDataType(PortType::Out).id == "TaskParameter") {
...
QVariant DataFlowGraphModel::nodeData(NodeId nodeId, NodeRole role) const
{
    ...
    case NodeRole::Expandable: {
            bool expandable = model->expandable();
            result = QVariant::fromValue(expandable);
        } break;
...
bool DataFlowGraphModel::setNodeData(NodeId nodeId, NodeRole role, QVariant value)
{
    ...
    case NodeRole::Style: {
            ...
            if (it == _models.end())
                return result;
            auto &model = it->second;
            QString style_string = value.value<QString>();

            NodeStyle nodeStyle(style_string);
    ...
    case NodeRole::Widget:
    {
        ...
        if (it == _models.end())
            return result;
        auto &model = it->second;
        bool flag = value.value<bool>();
        model->setWidgetExpand(flag);
        break;
    }
    }
