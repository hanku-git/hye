...
void NodeGraphicsObject::embedQWidget()
{
    ...
    //     //_originSize = _proxyWidget->size().toSize();
    // }
}
...
void NodeGraphicsObject::mouseDoubleClickEvent(QGraphicsSceneMouseEvent *event)
{
    ...
    // if (w && _graphModel.nodeData<bool>(_nodeId, NodeRole::Expandable)) {

    //     prepareGeometryChange();
    //     AbstractNodeGeometry &geometry = nodeScene()->nodeGeometry();
    //     bool flag = _nodeState.expanding();

    //     if (!flag) {
    //         _nodeState.setExpanding(true);       
    //         // _graphModel.setNodeData(_nodeId, NodeRole::Widget, QVariant(true));
    //         w->setFixedSize(_originSize);
    //         w->setVisible(true);
    //     } else {
    //         _nodeState.setExpanding(false);
    //         _originSize = w->size();
    //         w->setFixedSize(0, 0);
    //         w->setVisible(false);
    //     }        
