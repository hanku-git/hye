...
#include "ui_mainwindow_widgets_koras_cooking.h"
#include <QKeyEvent>
#include <QApplication>
MainWindow_widgetsKorasCooking::MainWindow_widgetsKorasCooking(MainWindow_node * taskWindow, QWidget* parent)
    : _taskWindow(taskWindow)
    , QMainWindow(parent)
    , ui(new Ui::MainWindow_widgetsKorasCooking)
    , isRecording(false)
{
    ui->setupUi(this);
    ...
    ///////////////////////////////////////////
    node_ = std::make_shared<rclcpp::Node>("koras_cooking_node");
    ...
    py_ui_subscription_ = node_->create_subscription<std_msgs::msg::String>(
        ...
        } else if(msg->data == "py_msg_do_task") {
            ROS_LOG_WARN("***** DO TASK! *****");
            //// Step 1) Generate Target Tasks
            // if (!cookingPlanner_) { setupPlanner(); }
            cookingPlanner_->generateCookingTask();
            //// Step 2) Do Target Tasks
            if (doTargetTask(cookingPlanner_->doosan_module_task_)) {
    ...
    // ROS2 spin을 백그라운드에서 실행할 쓰레드 생성
    ros_spin_thread_ = std::make_shared<std::thread>(&MainWindow_widgetsKorasCooking::rosSpin, this);
    ...
    QObject::connect(_taskWindow->getTaskManager()->_qnode, &QNode::sendLogMessageStr , this, &MainWindow_widgetsKorasCooking::sendLogMessageStr);
    ...
    connect(ui->pushButton_exit_program, &QPushButton::clicked, this,
            &MainWindow_widgetsKorasCooking::testBtnCallback);


    status_update_timer_ = new QTimer(this);
    connect(status_update_timer_, &QTimer::timeout, this, &MainWindow_widgetsKorasCooking::statusTimerCallback);
    ...
    connect(ui->pushButton_recording, &QPushButton::pressed, this, &MainWindow_widgetsKorasCooking::RecordingStartCallback);
    connect(ui->pushButton_recording, &QPushButton::released, this, &MainWindow_widgetsKorasCooking::RecordingStopCallback);
    QObject::connect(QApplication::instance(), SIGNAL(focusChanged(QWidget*, QWidget*)), this, SLOT(onFocusChanged(QWidget*, QWidget*)));

    connect(_taskWindow->getTaskManager()->_qnode, &QNode::UpdateLLMText, this, &MainWindow_widgetsKorasCooking::UpdateLLMText);
    connect(_taskWindow->getTaskManager()->_qnode, &QNode::UpdateVelocites, this, &MainWindow_widgetsKorasCooking::UpdateVelocites);
    connect(_taskWindow->getTaskManager()->_qnode, &QNode::UpdateImage, this, &MainWindow_widgetsKorasCooking::UpdateImage);
    connect(_taskWindow->getTaskManager()->_qnode, &QNode::UpdateWaypoints, this, &MainWindow_widgetsKorasCooking::UpdateWaypoints);
    connect(_taskWindow->getTaskManager()->_qnode, &QNode::UpdateGoalPose, this, &MainWindow_widgetsKorasCooking::UpdateGoalPose);
    connect(_taskWindow->getTaskManager()->_qnode, &QNode::LLMNodeReady, this, &MainWindow_widgetsKorasCooking::LLMNodeReadyCallback);


    timer_ = new QTimer(this);
    connect(timer_, &QTimer::timeout, this, &MainWindow_widgetsKorasCooking::timerCallback);
    timer_->start(100);
}



MainWindow_widgetsKorasCooking::~MainWindow_widgetsKorasCooking()
{
    if (rclcpp::ok()) {
        rclcpp::shutdown();
    }
...
void MainWindow_widgetsKorasCooking::closeEvent(QCloseEvent *bar)
{
    bar->ignore();
    return;
}

void MainWindow_widgetsKorasCooking::rosSpin()
{
    rclcpp::spin(node_);
}

void MainWindow_widgetsKorasCooking::python_to_cpp_callback(const std_msgs::msg::String &msg)
{
    ROS_LOG_WARN("Received from Python: '%s'", msg.data.c_str());
...
void MainWindow_widgetsKorasCooking::sendLogMessageStr(std::string &str) {
...
void MainWindow_widgetsKorasCooking::timerCallback() {
...
void MainWindow_widgetsKorasCooking::statusTimerCallback() {
...
void MainWindow_widgetsKorasCooking::testBtnCallback() {
...
void MainWindow_widgetsKorasCooking::keyPressEvent(QKeyEvent *event) {
...
void MainWindow_widgetsKorasCooking::keyReleaseEvent(QKeyEvent *event) {
    // if (event->key() == Qt::Key_Space && isRecording && _taskWindow->getTaskManager()->_qnode->is_llm_task_fin) {
    //     isRecording = false;
...
void MainWindow_widgetsKorasCooking::SetPoseCallback(const std::string& position) {
    double qd = 30;
    double qdd = 60;
...
void MainWindow_widgetsKorasCooking::llm_node_Callback(LLM_PARAM &llm_param) {
    // try {
...
void MainWindow_widgetsKorasCooking::target_obj_Callback() {
    // int new_gripper = 1;
    // int new_tip = 1;
...
void MainWindow_widgetsKorasCooking::GraspingTargetObject() {
...
void MainWindow_widgetsKorasCooking::GripperAttachCallback() {
...
void MainWindow_widgetsKorasCooking::GripperDetachCallback() {
    // QPushButton* button = qobject_cast<QPushButton*>(sender());
    // int gripper_type = 0; // 기본값
    // int tip_type = 0;
...
void MainWindow_widgetsKorasCooking::BoxSelectionCallback() {
    // QPushButton* button = qobject_cast<QPushButton*>(sender());
    // int boxNumber = 1; // 기본값
    // int palette_num = 5;
...
void MainWindow_widgetsKorasCooking::UpdateImage(const QImage &image, const QString &current_pass) {
    // if (current_pass == "A") {
...
void MainWindow_widgetsKorasCooking::UpdateWaypoints(const QString &coordinates,  const QString &current_pass) {
...
void MainWindow_widgetsKorasCooking::UpdateLLMText(const QString &target, const QString &goal, const QString &passes) {
...
void MainWindow_widgetsKorasCooking::UpdateVelocites(const QString &velocities) {

}

void MainWindow_widgetsKorasCooking::UpdateGoalPose(const geometry_msgs::msg::Pose2D &goal_pose) {
...
void MainWindow_widgetsKorasCooking::TargetObjectParamSetting() {
...
void MainWindow_widgetsKorasCooking::setupPlanner() {
    cookingPlanner_ = dynamic_cast<TaskPlannerCooking*>(_taskWindow->getTaskManager()->_qnode->task_planner_.get());
    if (!cookingPlanner_) {
        std::cerr << "task_planner_ is not TaskPlannerCooking!" << std::endl;
    }
}

bool MainWindow_widgetsKorasCooking::doTargetTask(const std::vector<UnitTask> &target_task) {
