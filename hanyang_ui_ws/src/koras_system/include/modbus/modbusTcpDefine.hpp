...
//#define LOG(fmt, ...) qDebug() << ("[ modbuspp ]" fmt, ##__VA_ARGS__)
...
inline bool modbus::modbus_connect()
{
    ...
    timeout.tv_sec = 2000; // after 20 seconds connect() will timeout
    timeout.tv_usec = 0;
#endif
...
inline void modbus::modbus_build_request(uint8_t *to_send, uint16_t address, int func) const
{
    ...
    //qDebug() << (uint8_t)(address >> 8u);
    //qDebug() << (uint8_t)(address & 0x00FFu);
