...
class ZividCaptureNode(Node):
    ...
    def __init__(self):
        super().__init__('scanner_node') # inheritance from ZIVID SDK
        self.node = rclpy.create_node('scanner_node_sub')
        self.init_set()
        ...
        print("...", flush=True)
        print("SCANNER_MODEL: ", flush=True)
        print(SCANNER_MODEL)
        print("...", flush=True)
        print("ZIVID Ready", flush=True)
    ...
    def run(self):
        ...
        while rclpy.ok():            
            ...
            if msg is not None:
                ...
                end_scan = time.time()      
                print(f"scan runtime: {end_scan - start_scan:.5f} sec", flush=True)    
    ...
    def build_cloud_msg(self, point_cloud, xyz, rgba):
        ...
        # e_t0 = time.time()      
        # print(f"build_cloud_msg (1-1): {e_t0 - s_t:.5f} sec", flush=True)    
        ...
        # e_t1 = time.time()      
        # print(f"build_cloud_msg (1-2): {e_t1 - e_t0:.5f} sec", flush=True)    
        ...
        # e_t2 = time.time()      
        # print(f"build_cloud_msg (1-3): {e_t2 - e_t1:.5f} sec", flush=True)    
        # msg.is_dense = False
        # msg.point_step = 28
        # msg.row_step = (msg.point_step * msg.width)
        # e_t3 = time.time()      
        # print(f"build_cloud_msg (1-4): {e_t3 - e_t2:.5f} sec", flush=True)    
        # points = np.block([xyz,rgba])
        # e_t4 = time.time()      
        # print(f"build_cloud_msg (1-5): {e_t4 - e_t3:.5f} sec", flush=True)   
        # # msg.data = np.asarray(points, np.float32).tobytes()
        # np_tmp = np.asarray(points, np.float32)
        # e_t5 = time.time()      
        # print(f"build_cloud_msg (1-6): {e_t5 - e_t4:.5f} sec", flush=True)  
        # msg.data = np_tmp.tobytes()
        # e_t6 = time.time()      
        # print(f"build_cloud_msg (1-7): {e_t6 - e_t5:.5f} sec", flush=True)  
    ...
    def capture(self):
        ...
        e_t0 = time.time()      
        print(f"camera.capture: {e_t0 - s_t:.5f} sec", flush=True)    
        
        cloud_msg = self.build_cloud_msg(point_cloud, xyz,rgba)
        e_t1 = time.time()      
        print(f"build_cloud_msg: {e_t1 - e_t0:.5f} sec", flush=True)    
        
        img_msg = self.cv_bridge.cv2_to_imgmsg(rgba, 'rgba8')
        e_t2 = time.time()      
        print(f"cv2_to_imgmsg: {e_t2 - e_t1:.5f} sec", flush=True)    
