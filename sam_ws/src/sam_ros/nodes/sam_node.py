...
sam_checkpoint = "/home/korasrobotics/[sam_weight]/sam_vit_h_4b8939.pth"  # sam_vit_h_4b8939 sam_vit_b_01ec64
model_type = "vit_h"
device = "cuda"
test = False
# save_img = True
...
class SamNode(object):
    def __init__(self):
        ...
        if self.no_detection_mode:
            ...
            #### Classifier
            model_path = self.node.declare_parameter('~model_path', '/home/korasrobotics/sam_ws/src/sam_ros/weight/final_model_new.pth').value
    ...
    def setSamParameters(self):
        ...
        with open ("/home/korasrobotics/sam_ws/src/sam_ros/config/sam_config.json", "r") as f:
            ...
            self.no_detection_mode = self.sam_config[self._target_name]['no_detection_mode']
            ...
            if self.no_detection_mode:
                ...
                print("-------------------\n", f"(no_detection_pixel_min_h: {self.sam_config[self._target_name]['no_detection_pixel_min_h']:d})\n", 
                                                f"(no_detection_pixel_min_v: {self.sam_config[self._target_name]['no_detection_pixel_min_v']:d})\n",
                                                f"(no_detection_pixel_length_h: {self.sam_config[self._target_name]['no_detection_pixel_length_h']:d})\n",
                                                f"(no_detection_pixel_length_v: {self.sam_config[self._target_name]['no_detection_pixel_length_v']:d})\n",
                                                f"(no_detection_pixel_radius: {self.sam_config[self._target_name]['no_detection_pixel_radius']:d})\n")
            else:
                print("AI Detection Mode!")
    ...
    def size_filter(self, masks, class_cnt):           
        ...
        for i in del_idx:
            if self._save_img:
                filename = "/home/korasrobotics/[zivid_scan_data]/Deleted_img/size-%d.png" % i
    ...
    def filtering(self, seq, masks, masks_area, boxes, class_cnt):      
        ...
        if self._do_overlap_filtering:
            ...
            # for i in del_idx:        
            #     if self._save_img:    
            #         filename = "/home/korasrobotics/[zivid_scan_data]/Deleted_img/overlap-%d.png" % i
            #         cv2.imwrite(os.path.join("", filename), masks[i]*255)  
